...
export default function DriverApp({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await fetch(${window.location.origin}/api/auth/login/driver', {
        method: 'POST',
        headers: {
  ...
  const sendLocationToBackend = async (location) => {
    ...
    try {
    const response = await fetch(${window.location.origin}/api/drivers/location', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
const toggleDriverStatus = async () => {
  if (driverStatus === 'offline') {
    try {
      ...
      const response = await fetch(${window.location.origin}/api/drivers/status', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
